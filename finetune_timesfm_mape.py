...
def validation_metrics_fn(pred, targets) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Calculates validation metrics (MSE, MAE, and MAPE)."""
    mse = torch.nn.functional.mse_loss(pred, targets)
    mae = torch.nn.functional.l1_loss(pred, targets)
    mape = torch.mean(torch.abs((targets - pred) / targets))
    return mse, mae, mape
...
def main():
    ...
    parser.add_argument("--val-metric", type=str, default="mape", choices=["mse", "mae", "mape"])
    ...
    for epoch in range(args.epochs):
        ...
        model.eval()
        total_val_mse, total_val_mae, total_val_mape = 0, 0, 0
        with torch.no_grad():
            for inputs, masks, targets in val_loader:
                inputs, masks, targets = inputs.to(device), masks.to(device), targets.to(device)
                with torch.cuda.amp.autocast():
                    preds = get_predictions(model, inputs, masks, args.horizon)
                    mse, mae, mape = validation_metrics_fn(preds, targets.to(device))
        ...
        print(f"Epoch {epoch+1}/{args.epochs} | Train Loss: {avg_train_loss:.6f} | Val MSE: {avg_val_mse:.6f} | Val MAE: {avg_val_mae:.6f} | Val MAPE: {avg_val_mape:.6f} | LR: {current_lr:.2e}")
        
        history.append({
            "epoch": epoch + 1, "train_loss": avg_train_loss, "val_mse": avg_val_mse, "val_mae": avg_val_mae, "val_mape": avg_val_mape, "lr": current_lr
        })
        with open(save_dir / "history.json", "w") as f:
            json.dump(history, f, indent=2)

        if args.val_metric == 'mae':
            current_val_metric = avg_val_mae
        elif args.val_metric == 'mape':
            current_val_metric = avg_val_mape
        else:
            current_val_metric = avg_val_mse
